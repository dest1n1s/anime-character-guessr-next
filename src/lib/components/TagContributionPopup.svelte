...
>
	...
	>
					...
					{#each Object.entries(tagGroups) as [groupName, tags]}
						<div class="tag-group">
							...
							<div class="flex flex-wrap gap-2">
								{#each tags as tag}
									<button
										class="cursor-pointer rounded-md px-3 py-1 text-sm transition-colors {selectedTags.includes(
											tag
										)
											? 'text-primary-800 bg-blue-100 hover:bg-blue-200'
											: 'bg-gray-100 text-gray-700 hover:bg-gray-200'} {totalTags >= MAX_TAGS &&
					...
					<div class="tag-group">
						...
						<div class="mb-2 flex gap-2">
							<input
								...
								maxlength={8}
								class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm {inputError
									? 'border-red-300 bg-red-50'
									: ''}"
								disabled={totalTags >= MAX_TAGS}
							/>
							<button
								class="cursor-pointer rounded-md bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700 disabled:opacity-50"
						...
						{#if customTags.length > 0}
							<div class="flex flex-wrap gap-2">
								{#each customTags as tag}
									<div
										class="text-primary-800 hover:bg-primary-200 flex cursor-pointer items-center gap-1 rounded-md bg-blue-100 px-2 py-1 text-sm"
									>
										<span>{tag}</span>
										<button
											class="text-primary-700 hover:bg-primary-300 ml-1 flex h-4 w-4 cursor-pointer items-center justify-center rounded-full bg-blue-200"
		...
		<div class="mt-6 flex justify-end">
			<button
				class="cursor-pointer rounded-md bg-blue-600 px-6 py-2 text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50"
